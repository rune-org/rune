...
Architecture:
    The CLI communicates with the Rune API server over HTTP/HTTPS.
    
    CLI (rune_cli) --HTTP--> API Server (services/api:8000) ---> Database
    
    The API server must be running for most commands to work.
    Local-only commands (config, some db operations via Docker) work offline.
"""
...
from rune_cli.auth.token_manager import get_token_manager
...
class APIClient:
    ...
    def request(
        ...
        try:
            ...
            raise ConnectionError(
                ...
                "The Rune API server must be running for this command.\n"
                "Start the API server with:\n"
                "  cd services/api && docker compose up -d\n"
                "  # or\n"
                "  cd services/api && python -m uvicorn src.app:app --reload\n\n"
                "Check your API URL with: rune config show"
            )
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "offline", "error": f"Cannot connect to {self.base_url}"}
        except requests.exceptions.Timeout:
