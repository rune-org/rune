...
class DockerClient:
    ...
    def exec_sql(
        ...
        Uses subprocess with stdin piping to avoid shell injection vulnerabilities.
        """
        # Build docker exec command with psql reading from stdin
        docker_cmd = [
            "docker", "exec", "-i",
            self.container_name,
        ...
        try:
            ...
            raise DockerError(f"SQL command timed out after {timeout} seconds")
        except subprocess.SubprocessError as e:
            raise DockerError(f"Docker SQL command failed: {e}")
    ...
    def exec_sql_file(
        ...
        try:
            ...
            raise DockerError(f"SQL file command timed out after {timeout} seconds")
        except subprocess.SubprocessError as e:
            raise DockerError(f"Docker SQL file command failed: {e}")
