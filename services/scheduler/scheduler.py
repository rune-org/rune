        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                self.pool = await asyncpg.create_pool(
                    ...
                    command_timeout=60,
                    server_settings={
                        'application_name': 'rune-scheduler'
                    }
                )
...
class MessageQueue:
    ...
    async def connect(self):
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                await self.channel.set_qos(prefetch_count=Config.MAX_CONCURRENT_EXECUTIONS)
                ...
                logger.info(f"RabbitMQ connection established (queue: {self.queue_name})")
                return
            except Exception as e:
    ...
    async def publish_workflow(self, workflow_id: int, schedule_id: int, workflow_data: Dict[str, Any]) -> bool:
        """
        ...
        try:
            ...
            logger.info(f"Published workflow execution: workflow_id={workflow_id}, schedule_id={schedule_id}")
            return True
    ...
    async def _check_and_execute_schedules(self):
        ...
        try:
            async with self.db_pool.pool.acquire() as conn:
                ...
                if not rows:
                    logger.debug(f"No schedules due (checked up to {look_ahead_time.isoformat()})")
                    return
                ...
                tasks = [
                    self._execute_schedule(row, conn)
                    for row in rows
                ]
