    ...
    async def resolve_workflow_credentials(
        ...
        for node in nodes:
            ...
            if "credentials" in node and isinstance(node["credentials"], dict):
                ...
                logger.info(f"Node {node.get('name')} has credentials reference: {cred_ref}")
                ...
                if "id" in cred_ref:
                    ...
                    logger.info(f"Resolved credentials for node {node.get('name')} with {len(credential_values)} values")
