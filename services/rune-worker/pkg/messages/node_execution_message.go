...
type NodeExecutionMessage struct {
	...
	WorkflowDefinition types.Workflow         `json:"workflow_definition"` // Complete workflow structure
...
func (m *NodeExecutionMessage) GetCurrentNodeDetails() (types.Node, error) {
	node, found := m.WorkflowDefinition.GetNodeByID(m.CurrentNode)
	if !found {
		return types.Node{}, fmt.Errorf("current_node %s not found in workflow", m.CurrentNode)
	}
	return node, nil
}
