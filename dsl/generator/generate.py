...
class DSLGenerator:
    ...
    def _generate_class(
        self, lang: str, class_name: str, fields: Dict[str, Any], description: str = ""
    ) -> str:
    ...
    def _generate_sanitization_method(
        self, lang: str, class_name: str, fields: Dict[str, Any]
    ) -> str:
        ...
        if lang == "typescript":
            ...
            for field_name, field_def in fields.items():
                ...
                if required:
                    lines.append(
                        f"  if (obj.{field_name} === undefined || obj.{field_name} === null) {{"
                    )
                    lines.append(
                        f'    errors.push("{class_name}.{field_name} is required");'
                    )
                    lines.append("  }")
                # Type checking
                if field_type == "string":
                    lines.append(
                        f'  if (obj.{field_name} !== undefined && typeof obj.{field_name} !== "string") {{'
                    )
                    lines.append(
                        f'    errors.push("{class_name}.{field_name} must be a string");'
                    )
                    lines.append("  }")
                elif field_type == "number":
                    lines.append(
                        f'  if (obj.{field_name} !== undefined && typeof obj.{field_name} !== "number") {{'
                    )
                    lines.append(
                        f'    errors.push("{class_name}.{field_name} must be a number");'
                    )
                    lines.append("  }")
                elif field_type == "boolean":
                    lines.append(
                        f'  if (obj.{field_name} !== undefined && typeof obj.{field_name} !== "boolean") {{'
                    )
                    lines.append(
                        f'    errors.push("{class_name}.{field_name} must be a boolean");'
                    )
                    lines.append("  }")
            lines.extend(
                [
            ...
            for field_name, field_def in fields.items():
                ...
                if required:
                    ...
                    lines.append(
                        f'            errors.append("{class_name}.{field_name} is required")'
                    )
                # Type checking
                if field_type == "string":
                    lines.append(
                        f"        if self.{python_field_name} is not None and not isinstance(self.{python_field_name}, str):"
                    )
                    lines.append(
                        f'            errors.append("{class_name}.{field_name} must be a string")'
                    )
                elif field_type == "number":
                    lines.append(
                        f"        if self.{python_field_name} is not None and not isinstance(self.{python_field_name}, (int, float)):"
                    )
                    lines.append(
                        f'            errors.append("{class_name}.{field_name} must be a number")'
                    )
                elif field_type == "boolean":
                    lines.append(
                        f"        if self.{python_field_name} is not None and not isinstance(self.{python_field_name}, bool):"
                    )
                    lines.append(
                        f'            errors.append("{class_name}.{field_name} must be a boolean")'
                    )
            lines.extend(["", "        return len(errors) == 0, errors"])
            return "\n".join(lines)
    ...
    def generate_typescript(self) -> str:
        ...
        for struct_name, struct_def in self.dsl_data["core_structures"].items():
            ...
            lines.append(
                self._generate_class("typescript", struct_name, fields, description)
            )
            lines.append("")
            lines.append(
                self._generate_sanitization_method("typescript", struct_name, fields)
            )
            lines.append("")
        ...
        if "nested_types" in self.dsl_data:
            ...
            for type_name, type_def in self.dsl_data["nested_types"].items():
                ...
                lines.append(
                    self._generate_class("typescript", type_name, fields, description)
                )
                lines.append("")
                lines.append(
                    self._generate_sanitization_method("typescript", type_name, fields)
                )
                lines.append("")
        ...
        for node_type, node_def in self.dsl_data["node_types"].items():
            ...
            if params:
                ...
                lines.append(
                    self._generate_class("typescript", class_name, params, description)
                )
                lines.append("")
                lines.append(
                    self._generate_sanitization_method("typescript", class_name, params)
                )
                lines.append("")
        ...
        lines.append("")
        lines.append(
            self._generate_sanitization_method("typescript", "BaseNode", base_fields)
        )
        lines.append("")
    ...
    def generate_python(self) -> str:
        ...
        for struct_name, struct_def in self.dsl_data["core_structures"].items():
            ...
            description = struct_def.get("description", "")
            lines.append(
                self._generate_class("python", struct_name, fields, description)
            )
            lines.append("")
            lines.append(
                self._generate_sanitization_method("python", struct_name, fields)
            )
            lines.append("")
        ...
        if "nested_types" in self.dsl_data:
            ...
            for type_name, type_def in self.dsl_data["nested_types"].items():
                ...
                description = type_def.get("description", "")
                lines.append(
                    self._generate_class("python", type_name, fields, description)
                )
                lines.append("")
                lines.append(
                    self._generate_sanitization_method("python", type_name, fields)
                )
                lines.append("")
        ...
        for node_type, node_def in self.dsl_data["node_types"].items():
            ...
            if params:
                ...
                description = node_def.get("description", "")
                lines.append(
                    self._generate_class("python", class_name, params, description)
                )
                lines.append("")
                lines.append(
                    self._generate_sanitization_method("python", class_name, params)
                )
                lines.append("")
        ...
        lines.append("")
        lines.append(
            self._generate_sanitization_method("python", "BaseNode", base_fields)
        )
        lines.append("")
        ...
        for node_type, node_def in self.dsl_data["node_types"].items():
            ...
            lines.append(
                f"    credential_type: Optional[list[str]] = {credential_type_default}"
            )
            lines.append("")
            ...
            lines.append(
                f'                errors.append(f"{class_name}.credentials.type must be one of {{self.credential_type}}")'
            )
            lines.append("")
    ...
    def generate_go(self, package_name="dsl"):
        ...
        if "nested_types" in self.dsl_data:
            ...
            for type_name, type_def in self.dsl_data["nested_types"].items():
                ...
                lines.append("")
                lines.append(
                    self._generate_sanitization_method("go", type_name, fields)
                )
                lines.append("")
        ...
        for node_type, node_def in self.dsl_data["node_types"].items():
            ...
            if params:
                ...
                description = node_def.get("description", "")
                lines.append(
                    self._generate_class("go", class_name, params, description)
                )
                lines.append("")
                lines.append(
                    self._generate_sanitization_method("go", class_name, params)
                )
                lines.append("")
        ...
        for node_type, node_def in self.dsl_data["node_types"].items():
            ...
            if credential_type is None:
                ...
                lines.append(
                    f'var {node_type.upper()}_CREDENTIAL_TYPE []string = []string{{"{'", "'.join(credential_type)}"}}'
                )
            else:
                lines.append(
                    f'var {node_type.upper()}_CREDENTIAL_TYPE []string = []string{{"{credential_type}"}}'
                )
        lines.append("")
    ...
    def generate_all(self):
        ...
        web_ts_path = self.repo_root / "apps/web/src/lib/workflow_dsl_generated.ts"
        if (self.repo_root / "apps/web").exists():
        ...
        api_py_path = self.repo_root / "services/api/src/dsl/workflow_dsl_generated.py"
        if (self.repo_root / "services/api").exists():
            # Ensure directory exists
        ...
        worker_go_path = self.repo_root / "services/rune-worker/pkg/dsl_generated/workflow_dsl_generated.go"
        if (self.repo_root / "services/rune-worker").exists():
