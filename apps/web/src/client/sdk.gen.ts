 ...
 *
 * Search for users available for sharing credentials/workflows. Returns minimal user info, excludes the current user. Use the 'search' parameter to filter by name or email.
 */
export const listUsersForSharingUsersSharingGet = <ThrowOnError extends boolean = false>(options?: Options<ListUsersForSharingUsersSharingGetData, ThrowOnError>) => (options?.client ?? client).get<ListUsersForSharingUsersSharingGetResponses, ListUsersForSharingUsersSharingGetErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/users/sharing',
    ...options
});
