...
type Props = {
  node: Node<DateTimeSubtractData>;
  updateData: ReturnType<typeof useUpdateNodeData>;
  isExpanded: boolean;
};

export function DateTimeSubtractInspector({ node, updateData, isExpanded }: Props) {
  const timezone = node.data.timezone ?? "UTC";
  const format = node.data.format ?? "2006-01-02T15:04:05Z07:00";

  const update = (updater: (data: DateTimeSubtractData) => DateTimeSubtractData) => {
    updateData(node.id, "dateTimeSubtract", updater);
  };
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <div>
          ...
          <Select
            ...
            onValueChange={(value) =>
              update((d) => ({ ...d, unit: value as DateTimeSubtractData["unit"] }))
            }
          >
      ...
      {isExpanded && (
        <div className="rounded-[calc(var(--radius)-0.25rem)] border border-border/40 bg-muted/20 p-2 text-xs text-muted-foreground/70">
          Subtracts the chosen duration from the starting date (or from the current time when
          empty).
        </div>
      )}
    </div>
