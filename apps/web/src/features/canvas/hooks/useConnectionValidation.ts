...
import { switchFallbackHandleId, switchRuleHandleId } from "../utils/switchHandles";
...
export function useConnectionValidation(opts: {
  ...
  return useCallback((connection: Edge | Connection) => {
    ...
    if (sourceNode.type === "if") {
      return !existingSourceEdges.some((edge) => edge.sourceHandle === sourceHandle);
    }

    // For "switch" nodes: validate handle exists, then max 1 edge per handle
    if (sourceNode.type === "switch") {
      const rules = Array.isArray(sourceNode.data.rules) ? sourceNode.data.rules : [];
