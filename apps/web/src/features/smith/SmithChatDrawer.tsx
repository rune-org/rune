            ...
            {messages.length === 0 && (
              <div className="flex flex-col items-center justify-center py-12 text-center opacity-60 select-none">
                ...
                <p className="max-w-[260px] text-xs text-muted-foreground">
                  Describe your workflow logic, and I&apos;ll handle the nodes
                  and connections.
                </p>
              </div>
            )}
                    ...
                    {msg.role === "tool_call" ? (
                      ...
                      >
                        {msg.role === "smith" ? (
                          <MarkdownRenderer
                            content={msg.content}
                            className="prose-sm prose-invert [&>*:first-child]:mt-0 [&>*:last-child]:mb-0"
                          />
                        ) : (
                          msg.content
                        )}
                          ...
                          <div className="border-t border-border/40 bg-background/40 px-3 py-2">
                            <ul className="space-y-1.5">
                              {msg.trace.map((step, stepIdx) => (
                                <li
                                  key={stepIdx}
                                  className="flex gap-2 text-[11px] text-muted-foreground/80 select-text"
                                >
                                  <span className="font-mono opacity-50 select-none">
                                    {stepIdx + 1}.
                                  </span>
                                  <span>{step}</span>
                                </li>
