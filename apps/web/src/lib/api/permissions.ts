...
export async function shareWorkflow(
  ...
  try {
    ...
    const message =
      err?.detail && typeof err.detail === "string" ? err.detail : "Failed to share workflow";
 ...
 */
export async function revokeWorkflowAccess(workflowId: string, userId: number): Promise<void> {
  try {
    ...
    const err = error as Record<string, unknown> | undefined;
    const message =
      err?.detail && typeof err.detail === "string" ? err.detail : "Failed to revoke access";
    throw new Error(message);
  }
}
 ...
 */
export async function listWorkflowPermissions(workflowId: string): Promise<WorkflowPermission[]> {
  try {
    ...
    const err = error as Record<string, unknown> | undefined;
    const message =
      err?.detail && typeof err.detail === "string" ? err.detail : "Failed to fetch permissions";
    throw new Error(message);
  }
}
...
export async function updateWorkflowUserRole(
  ...
  try {
    ...
    const err = error as Record<string, unknown> | undefined;
    const message =
      err?.detail && typeof err.detail === "string" ? err.detail : "Failed to update role";
    throw new Error(message);
  }
}
