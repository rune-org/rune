...
3. **Set up your environment**:
   ...
   ```
   Alternatively, you can start the infrastructure and services manually with `make dev-infra-up`, then run individual services with `make web-dev`, `make api-dev`, `make worker-dev`, `make rtes-dev`, `make archivist-dev`, or run everything in production mode via Docker with `make up`.
